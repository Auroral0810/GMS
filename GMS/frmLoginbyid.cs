...
namespace GMS
{
    public partial class frmLoginbyid : Form
    {
        public string LoginType  = "Id";
        ...
        private bool usercheck()
        {
            ...
            using (SqlConnection conn = new SqlConnection(DBconn.connstring))
            {
                ...
                try
                {
                    ...
                    // 构建查询语句
                    string sqlstr = "SELECT COUNT(*) FROM Login WHERE id = @ID AND pwd = @Password AND usertype = @UserType";
        ...
        private bool inputCheck()
        {
            ...
            if (string.IsNullOrEmpty(this.nameinput.Text) || string.IsNullOrEmpty(this.pwdinput1.Text) || this.nameinput.Text == "用户名")
            {
                MessageBox.Show("未输入用户名或密码！");
            }
            else if (!Regex.IsMatch(this.nameinput.Text, pattern))// 使用正则表达式进行匹配
            {
                MessageBox.Show("用户名只能包含字母、数字和下划线！");
            }
                ...
                if (this.usercheck())
                {
                    ...
                    timer.Tick += async (s, ev) =>
                    {
                        ...
                        if (typelist.Text == "管理员")
                        {
                            ...
                            await Loading();
                            // 创建并显示 frmAdmin 窗体
                            frmAdmin frmadmin = new frmAdmin(id, LoginType);
                            // 关闭当前的 frmLogin 窗口
                            this.Hide();
                            frmadmin.Show();
                        ...
                        else if (typelist.Text == "用户")
                        {
                            ...
                            await Loading();
                            // 创建并显示 frmAdmin 窗体
                            frmUser user = new frmUser(id, LoginType);
                            // 关闭当前的 frmLogin 窗口
                            this.Hide();
                            // 创建并显示 frmAdmin 窗体
                            user.ShowDialog();
                        ...
                        else if (typelist.Text == "教练")
                        {
                            ...
                            // 异步运行 Loading() 方法
                            await Loading();
                            frmCoach coach = new frmCoach(id, LoginType);
                            // 关闭当前的 frmLogin 窗口
                            this.Hide();
                            coach.ShowDialog();
        ...
        private void phoneSign_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            this.Hide();
            // 创建并显示 frmForgetpwd 对话框
            using (frmLoginbyphone loginbyphone = new frmLoginbyphone())
            {
                // 使用 ShowDialog 显示窗体，这样它将以模态对话框的形式出现
                loginbyphone.ShowDialog(this);
            }
        }
    }
}
