...
namespace GMS
{
    public partial class frmForgetPwdByUsername : Form
    {
        ...
        private void verifybtn_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection connection = new SqlConnection(DBconn.connstring))
            {
                try
                {
                    ...
                    // 查找用户
                    SqlCommand cmdFindUser = new SqlCommand("SELECT id FROM Login WHERE usertype = '用户' AND id = @FindId", connection, transaction);
        ...
        {
            // 获取用户输入
            string findId = findid.Text.Trim(); // 用户名
            ...
            // 使用正则表达式进行匹配
            if (!Regex.IsMatch(findId, pattern))
            {
        ...
        private void changeway_Click(object sender, EventArgs e)
        {
            ...
            using (frmForgetPwdByPhone pwdByPhone = new frmForgetPwdByPhone())
            {
                // 使用 ShowDialog 显示窗体，这样它将以模态对话框的形式出现
                pwdByPhone.ShowDialog(this);
            }
        }
    }
}
